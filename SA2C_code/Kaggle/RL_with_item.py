...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.item_features = tf.placeholder(tf.float32, [self.item_num, self.item_feature_size]) # item_num * item_feature_size
            ...
            self.output1 = _layers.fully_connected(self.states_hidden, self.item_num,
                                                            activation_fn=None)  # all q-values

            self.output2 = _layers.fully_connected(self.states_hidden, self.item_num,
                                                             activation_fn=None, scope="ce-logits")  # all ce logits


            self.item_feature_b = _layers.fully_connected(self.item_features, 1, activation_fn=None)

            self.item_feature_w = _layers.fully_connected(self.item_features, self.hidden_size, activation_fn=None)

            self.output3 = tf.matmul(self.item_feature_w, tf.transpose(self.states_hidden)) + self.item_feature_b

            # print(self.item_feature_w.shape)
            # print(self.states_hidden.shape)
            # print(self.item_feature_b.shape)
            # print(self.output3.shape)

            self.output3 = tf.transpose(self.output3)

            self.output4 = self._lambda * self.output3 + (1 - self._lambda) * self.output2
            ...
            ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.output4)
...
def calculate_hit(prediction,topk,true_items,rewards,r_click,total_reward,hit_click,ndcg_click,hit_purchase,ndcg_purchase):
    # sorted_list=np.argsort(prediction)
    k = max(topk)
    ind = np.argpartition(prediction, -k)[:, -k:]
    row_indices = np.arange(prediction.shape[0])[:, None]
    ind = ind[row_indices, np.argsort(prediction[row_indices, ind])]
    for i in range(len(topk)):
        k = topk[i]
        # sorted_list = np.argpartition(prediction, -k)
        rec_list = ind[:, -k:]
...
def evaluate(sess):
    ...
    while evaluated<len(eval_ids):
        ...
        prediction=sess.run(QN_1.output4, feed_dict={QN_1.inputs: states,QN_1.len_state:len_states,QN_1.item_features: item_features,QN_1.is_training:False})
        ...
        # print(f"running time: {t1-t0}, evaluated:{evaluated}")
    t1 = time.time()
    print('#############################################################')
...
if __name__ == '__main__':
    ...
    with open('data/item_feature_100.npy', 'rb') as f:
        item_features = np.load(f)
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                if total_step % 200 == 0:
                    t1 = time.time()
                    print(f"the loss in {total_step}th batch is: {loss} the time is {t1-t0}")
                    t0 = time.time()
