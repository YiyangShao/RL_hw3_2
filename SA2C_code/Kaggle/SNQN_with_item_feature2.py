...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.item_features = tf.placeholder(tf.int32, [self.item_num + 1, self.item_feature_size]) # item_num * item_feature_size
            ...
            self.item_feature_b = _layers.fully_connected(combined_output, 1, activation_fn=None)
            
            self.item_feature_w = _layers.fully_connected(combined_output, self.hidden_size, activation_fn=None)
...
def evaluate(sess):
    ...
    while evaluated<len(eval_ids):
        ...
        for i in range(batch):
            ...
            evaluated+=1
        prediction=sess.run(QN_1.output4, feed_dict={QN_1.inputs: states,QN_1.len_state:len_states,target_QN.item_features: item_features,QN_1.is_training:False})
        sorted_list=np.argsort(prediction)
        # sorted_list = np.argpartition(prediction, -max(topk))
